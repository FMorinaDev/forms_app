...
class BlocCounterView extends StatelessWidget {
  ...
  void refresh(BuildContext context){
    context.read<CounterBloc>().add(Refresh());
  }

  @override
  Widget build(BuildContext context) {
    final transaction = context.watch<CounterBloc>().state.transactionCount;
    return Scaffold(
      appBar: AppBar(
        title: Text('Bloc Counter $transaction'),
        actions: [
          IconButton(
            onPressed: () => refresh(context), icon: const Icon(Icons.refresh_outlined))
        ],
      ),
      body: Center(
